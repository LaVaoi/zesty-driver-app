...
class LocationTrackingService : Service() {
    ...
    companion object {
        ...
        // Persisted so tracking can be restored after a full device reboot
        // (BOOT_COMPLETED receiver reads these — see BootReceiver.kt)
        const val PREFS_NAME = "zesty_location_tracking_prefs"
        const val KEY_TRACKING = "is_tracking"
        const val KEY_TOKEN = "saved_token"
        const val KEY_API_URL = "saved_api_url"
    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        when (intent?.action) {
            ACTION_START -> {
                ...
                persistTrackingState(isTracking = true, token = driverToken, apiUrl = apiUrl)
                startAsForegroundService()
                startLocationUpdates()
            }
            ACTION_STOP -> {
                Log.d(TAG, "🔴 Stopping service")
                persistTrackingState(isTracking = false, token = "", apiUrl = "")
                stopLocationUpdates()
    ...
    private fun persistTrackingState(isTracking: Boolean, token: String, apiUrl: String) {
        ...
        prefs.edit()
            ...
            .apply()
        Log.d(TAG, "💾 Persisted tracking state: isTracking=$isTracking")
    }
